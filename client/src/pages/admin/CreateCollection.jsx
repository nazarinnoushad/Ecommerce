...
//  handle form for create collection
  ...
  const handleSubmit = async(e)=>{
    e.preventDefault()
    try{
const {data}= await axios.post("http://localhost:4000/api/v1/collection/create-collection",{name})
...
const deleteCollection = async (id) =>{
  try{
const {data} = await axios.delete(`http://localhost:4000/api/v1/collection/delete-collection/${id}`)
if (data && data.success) {
  toast.success(data.message)
  getAllCollection()
}
...
const handleUpdate = async(e)=>{
   e.preventDefault()
  try{
const {data} = await axios.put(`http://localhost:4000/api/v1/collection/update-collection/${selected._id}`,{name:updatedName})
if (data&& data.success) {
  setSelected(null)
  setUpdatedName("")
  handleCancel()
